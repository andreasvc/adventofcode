...
def doexplode(node, cur, maxdepth, digits=re.compile(r'\d+')):
	...
	def findsibling(node, x, right):
		if isinstance(node, int):
			return node + x
		node[right] = findsibling(node[right], x, right)
		return node
...
def _day23(s, part2=False):
	...
	"""
	def end(state):
		return all(x == goalcol[a] for y, x, a in state)

	def destfree(y, x, a, state):
		if any(xx == goalcol[a] and aa != a for _, xx, aa in state):
			return False  # other amphipods in goal room
		xmin, xmax = min(x, goalcol[a]), max(x, goalcol[a])
		if any(yy == 1 and xmin <= xx <= xmax and xx != x
				for yy, xx, _ in state):
			return False  # obstruction in hallway
		return True

	def moves(state):
		s = {(yy, xx) for yy, xx, _ in state}
		for y, x, a in state:
			if (y - 1, x) in s or all(
					xx == goalcol[aa] for _, xx, aa in state if aa == a):
				continue  # obstruction above or room already done?
			elif x != goalcol[a] and destfree(y, x, a, state):
				xx = goalcol[a]
				yy = max(yy for yy in range(2, 6 if part2 else 4)
						if (yy, xx) not in s)
				yield y, x, yy, xx, a
			elif (y, x, a) in initialstate:
				# moves to the left
				for xx in (8, 6, 4, 2, 1):
					if xx < x:
						if (1, xx) in s:
						...
						yield y, x, 1, xx, a
				# moves to the right
				for xx in (4, 6, 8, 10, 11):
					if xx > x:
						if (1, xx) in s:
							break
						yield y, x, 1, xx, a

	def cost(y, x, yy, xx, a):
		return (y - 1 + abs(xx - x) + yy - 1) * costs[a]

	lines = s.splitlines()
	if part2:
		lines = lines[:3] + ['  #D#C#B#A#', '  #D#B#A#C#'] + lines[3:]
	state = tuple((y, x, 'ABCD'.index(a))
	...
	seen = {}
	while agenda:
		score, state = heappop(agenda)
		if end(state):
			return score
		for y, x, yy, xx, a in moves(state):
			newstate = tuple(
					(yy, xx, a) if y1 == y and x1 == x
					else (y1, x1, a1)
					for y1, x1, a1 in state)
			newscore = score + cost(y, x, yy, xx, a)
			if newstate not in seen or newscore < seen[newstate]:
				heappush(agenda, (newscore, newstate))
				seen[newstate] = newscore
...
def day23b(s):
	return _day23(s, part2=True)


def _day24_codegen(s):
	print('w = x = y = z = 0')
	n = 0
	for a in s.splitlines():
		b = a.split()[1]
		c = a.split()[2] if a.count(' ') > 1 else None
		if a.startswith('inp'):
			print('%s = inp[%d]' % (b, n))
			n += 1
		elif a.startswith('add'):
			print('%s += %s' % (b, c))
		elif a.startswith('mul'):
			print('%s *= %s' % (b, c))
		elif a.startswith('div'):  # int(a / b) ensures rounding towards 0
			print('%s = int(%s / %s)' % (b, b, c))
		elif a.startswith('mod'):
			print('%s %%= %s' % (b, c))
		elif a.startswith('eql'):
			print('%s = %s == %s' % (b, b, c))


def _day24_decompiled(inp):
	"""A decompiled version of the code generated by the above function.
	Must ensure that each condition is false.
	Conditions depend on the current input and the value of z, which is a stack
	of values based on the inputs.
	Normally the condition depends on the directly preceding input, but each
	division by 26 makes the condition depend on an input further back,
	throwing away the most recently added value."""
	i1, i2, i3, i4, i5, i6, i7, i8, i9, i10, i11, i12, i13, i14 = inp
	...
	x = z % 26 - 2
	z = int(z / 26)  # i7
	if x != i11: # i11 should be equal to i10
		z *= 26
		z += i11 + 11
		print("i11")
	...
	x = z % 26 - 1
	z = int(z / 26)  # i2
	if x != i12: # i12 should be equal to i7 + 8
		z *= 26
		z += i12 + 12
		print("i12")
	...
	x = z % 26 - 4
	z = int(z / 26)  # i1
	if x != i13: # i13 should be equal to i2 + 7
		z *= 26
		z += i13 + 3
		print("i13")
	...
	x = z % 26 - 12
	z = int(z / 26)  # 0
	if x != i14: # i14 should be equal to i1 - 8
		z *= 26
		z += i14 + 13
		print("i14")
...
def day24a(s):
	solution = [
			...
			9, 1, 4, 9, 9,
			9, 9, 9, 1]
	assert _day24_decompiled(solution)
